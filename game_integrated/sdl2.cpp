	...
	bool peek_message(event_t& e) {
		...
		while (SDL_PollEvent(&sdl_e)) {
			...
			case SDL_MOUSEBUTTONDOWN:
				...
				log("[SDL] MB_DOWN btn=%d x=%d y=%d clicks=%d\n", e.button, e.mouse_x, e.mouse_y, e.clicks);
				return true;
			case SDL_MOUSEBUTTONUP:
				...
				log("[SDL] MB_UP btn=%d x=%d y=%d clicks=%d\n", e.button, e.mouse_x, e.mouse_y, e.clicks);
				return true;
			case SDL_KEYDOWN:
