    ...
    static void put_pixel_rgba(uint32_t* data, int pitch_words, int x, int y, uint32_t color) {
        if (x < 0 || y < 0) return;
    ...
    // 5x7 uppercase ASCII bitmap for a subset of letters and symbols
    static const uint8_t* glyph5x7(char c) {
        // Each glyph is 5 columns of 7-bit height (LSB at top)
        static const uint8_t map[][6] = {
            // ' ' 32
            {32, 0,0,0,0,0},
            // ':' 58
            {58, 0x00,0x24,0x00,0x24,0x00},
            // '/' 47
            {47, 0x02,0x04,0x08,0x10,0x20},
            // '0'
            ...
            // 'R'
            {'R', 0x7F,0x48,0x4C,0x4A,0x71},
            // 'S'
            {'S', 0x62,0x51,0x49,0x49,0x46},
            // 'T'
    ...
    static void draw_glyph5x7(uint32_t* data, int pitch_words, int x, int y, uint32_t color, char c) {
        ...
        for (int i = 0; i < 5; ++i) {
            uint8_t bits = col[i];
            for (int j = 0; j < 7; ++j) {
                // Draw top-to-bottom using MSB-at-top to avoid flipped appearance
                if (bits & (1u << (6 - j))) put_pixel_rgba(data, pitch_words, x + i, y + j, color);
            }
        }
    }
    ...
    static void draw_text5x7_scaled(uint32_t* data, int pitch_words, int x, int y, const std::string& text, uint32_t color, int scale) {
        ...
        for (char ch : text) {
            ...
            for (int gx = 0; gx < 5; ++gx) {
                uint8_t col = g[gx+1];
                for (int gy = 0; gy < 7; ++gy) if (col & (1u << gy)) {
                    int px = cx + gx * scale;
                    int py = y + gy * scale;
    ...
    static const glyph8x13_t* glyph8x13(char c) {
        ...
        static const glyph8x13_t map[] = {
            ...
            {'S', {0x3E,0x63,0x60,0x60,0x3C,0x1E,0x03,0x03,0x03,0xC3,0x66,0x3C,0}},
...
//			}
	...
	void draw_selection_circle(const sprite_t* sprite, const unit_t* u, uint8_t* data, size_t pitch) {
	...
	void draw_sprite(const sprite_t* sprite, uint8_t* data, size_t pitch) {
		...
		for (auto* image : ptr(reverse(sprite->images))) {
			...
			if (draw_selection_u && image->modifier != 10) {
				draw_selection_circle(sprite, draw_selection_u, data, pitch);
	...
	void draw_minimap(uint8_t* data, size_t pitch) {
		...
		line_rectangle(data, pitch, {area.from - xy(1, 1), area.to + xy(1, 1)}, 0);

		uint8_t* p = data + pitch * (size_t)area.from.y + (size_t)area.from.x;

		size_t pitchw = pitch - game_st.map_tile_width;
		...
		for (size_t i = 12; i != 0;) {
			...
			for (unit_t* u : ptr(st.player_units[i])) {
				...
				fill_rectangle(data, pitch, unit_area, color);
			}
		}
	...
	void draw_ui(uint8_t* data, size_t pitch) {
	...
	void update() {
		...
		if (build_preview_active) {
			...
			xy screen_c = build_preview_pos - screen_pos;
			int w = build_preview_size.x;
			int h = build_preview_size.y;
			int left = screen_c.x - w / 2;
			int top = screen_c.y - h / 2;
			int right = left + w;
			int bottom = top + h;
		...
		// Draw toast on top-left
		if (toast_frames_left > 0 && !toast_text.empty()) {
			uint32_t* data = (uint32_t*)rgba_surface->lock();
			int pitchw = rgba_surface->pitch / 4;
			int x0 = 12, y0 = 12;
			int w = (int)toast_text.size() * 6 + 12;
			int h = 14;
			// bright background for readability
			uint32_t bg = 0xCCFFFF80u; // light yellow
			for (int yy = 0; yy < h; ++yy) for (int xx = 0; xx < w; ++xx) put_pixel_rgba(data, pitchw, x0 + xx, y0 + yy, bg);
			// border
			uint32_t border = 0xFF000000u;
			for (int xx = 0; xx < w; ++xx) { put_pixel_rgba(data, pitchw, x0 + xx, y0, border); put_pixel_rgba(data, pitchw, x0 + xx, y0 + h - 1, border); }
			for (int yy = 0; yy < h; ++yy) { put_pixel_rgba(data, pitchw, x0, y0 + yy, border); put_pixel_rgba(data, pitchw, x0 + w - 1, y0 + yy, border); }
			// text shadow + black text
			draw_text5x7(data, pitchw, x0 + 5, y0 + 4, toast_text, 0x7F000000u);
			draw_text5x7(data, pitchw, x0 + 4, y0 + 3, toast_text, 0xFF000000u);
			rgba_surface->unlock();
			--toast_frames_left;
		}
        ...
        if (hud_show) {
            ...
            if (data) {
                ...
                uint32_t bg = 0x60000000u; // semi-transparent black
                for (int yy = 0; yy < bg_h; ++yy) for (int xx = 0; xx < tw; ++xx) put_pixel_rgba_blend(data, pitchw, x0 + xx, y0 + yy, bg);
                // outline (8-connected) using semi-transparent black
                ...
                draw_text8x13_scaled(data, pitchw, x0,   y0+1, s, outline, scale);
                // main text
                draw_text8x13_scaled(data, pitchw, x0, y0, s, 0xFFFFFFFFu, scale);
                rgba_surface->unlock();
            }
        }
