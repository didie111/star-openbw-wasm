...
void player_input_handler::handle_event(const native_window::event_t& e, xy screen_pos) {
    ...
    switch (e.type) {
        case native_window::event_t::type_mouse_motion:
            ...
            // 마우스 이동 시 버튼 상태로 동기화: 우클릭 UP 이벤트 누락 대비
            // button_state: 1=left, 2=right, 4=middle
            if (right_button_down && (e.button_state & 2) == 0) {
                log("[RC_SYNC] motion sync: right_button_down -> false (btn_state=%d)\n", e.button_state);
        ...
        case native_window::event_t::type_mouse_button_down:
            ...
            if (e.button == 1) {
                ...
                log("[RC_EVENT] DOWN at=(%d,%d) rb_down=%d clicks=%d\n", mouse_pos.x, mouse_pos.y, (int)right_button_down, e.clicks);
                if (!right_button_down) right_button_down = true;
                else { log("[RC_SUPPRESS] ignored DOWN because rb_down already true\n"); }
            }
            break;
        ...
        case native_window::event_t::type_mouse_button_up:
            ...
            if (e.button == 1) {
                ...
                log("[RC_EVENT] UP at=(%d,%d) rb_down->false\n", mouse_pos.x, mouse_pos.y);
                right_button_down = false;
            }
            break;
...
void player_input_handler::on_right_click(xy pos) {
    ...
    auto adjust_pos_if_needed = [&](int pid, xy orig_pos, unit_t* orig_target) {
        ...
        if (!action_st.selection[pid].empty()) primary = action_st.selection[pid].front();
        if (primary) {
            // 자기 자신 클릭 또는 거의 같은 위치 클릭 시 약간 오프셋
    ...
    if (allow_enemy_control) {
        ...
        for (int pid = 0; pid < 8; ++pid) if (!action_st.selection[pid].empty()) {
            ...
            try {
                ...
                xy use_pos = ap.first;
                // 자기 자신 클릭 여부 판단 (선택된 어떤 유닛이라도 해당되면 self 처리)
                bool self_click = false;
                if (target && !action_st.selection[pid].empty()) {
                    for (auto* su : action_st.selection[pid]) { if (su == target) { self_click = true; break; } }
                }
                switch (command_mode) {
                    case CommandMode::Attack:
                        log("  [명령] 강제 공격 (A키 모드)\n");
                        ui::log("[RC] AttackMove to (%d,%d)\n", use_pos.x, use_pos.y);
                        actions.action_order(pid, actions.get_order_type(Orders::AttackMove), use_pos, target, nullptr, shift_pressed);
                        break;
                    case CommandMode::Move:
                        log("  [명령] 강제 이동 (M키 모드)\n");
                        ui::log("[RC] Move to (%d,%d)\n", use_pos.x, use_pos.y);
                        actions.action_order(pid, actions.get_order_type(Orders::Move), use_pos, nullptr, nullptr, shift_pressed);
                        break;
                    case CommandMode::Patrol:
                        log("  [명령] 정찰 (P키 모드)\n");
                        actions.action_order(pid, actions.get_order_type(Orders::Patrol), use_pos, nullptr, nullptr, shift_pressed);
                        break;
                    default:
                        if (self_click) {
                            log("  [명령] self-target 감지 -> 기본 대신 Move 강제\n");
                            ui::log("[RC] Move (self) to (%d,%d)\n", use_pos.x, use_pos.y);
                            actions.action_order(pid, actions.get_order_type(Orders::Move), use_pos, nullptr, nullptr, shift_pressed);
                        } else {
                            log("  [명령] 기본 명령 (스마트 명령)\n");
        ...
        // 단일 오너: 명령 모드에 따라 다른 동작 (목표 좌표 조정 포함)
        auto ap = adjust_pos_if_needed(player_id, pos, target);
        xy use_pos = ap.first;
        bool self_click = false;
        if (target && !action_st.selection[player_id].empty()) {
            for (auto* su : action_st.selection[player_id]) { if (su == target) { self_click = true; break; } }
        }
        switch (command_mode) {
        case CommandMode::Attack:
            log("  [명령] 강제 공격 (A키 모드)\n");
            ui::log("[RC] AttackMove to (%d,%d)\n", use_pos.x, use_pos.y);
            actions.action_order(player_id, actions.get_order_type(Orders::AttackMove), use_pos, target, nullptr, shift_pressed);
            command_mode = CommandMode::Normal;
            break;
        ...
        case CommandMode::Move:
            log("  [명령] 강제 이동 (M키 모드)\n");
            ui::log("[RC] Move to (%d,%d)\n", use_pos.x, use_pos.y);
            actions.action_order(player_id, actions.get_order_type(Orders::Move), use_pos, nullptr, nullptr, shift_pressed);
            command_mode = CommandMode::Normal;
            break;
            
        case CommandMode::Patrol:
            log("  [명령] 정찰 (P키 모드)\n");
            actions.action_order(player_id, actions.get_order_type(Orders::Patrol), use_pos, nullptr, nullptr, shift_pressed);
            command_mode = CommandMode::Normal;
            break;
        ...
        default:
            ...
            // 좌표 유효성 검사
            if (use_pos.x < 0 || use_pos.y < 0 || use_pos.x > 32768 || use_pos.y > 32768) {
                log("  [오류] 잘못된 좌표 감지! 명령 무시\n");
                log("[우클릭 디버그 끝]\n\n");
                return;
            }
            try {
                if (self_click) actions.action_order(player_id, actions.get_order_type(Orders::Move), use_pos, nullptr, nullptr, shift_pressed);
                else actions.action_default_order(player_id, use_pos, target, nullptr, shift_pressed);
            } catch (const std::exception& ex) {
                log("[CRASH_ORDER] default pid=%d ex=%s pos=(%d,%d)\n", player_id, ex.what(), use_pos.x, use_pos.y);
            } catch (...) {
                log("[CRASH_ORDER] default pid=%d unknown pos=(%d,%d)\n", player_id, use_pos.x, use_pos.y);
            }
            break;
        }
    }
